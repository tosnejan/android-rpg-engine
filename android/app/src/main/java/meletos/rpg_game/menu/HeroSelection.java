        ...
        while (isAlive){
            if (moveLeft){
                ...
                for (int i = 1; i < 9; i++) {
                    ...
                    buttons[0] = new Button((int) (baseX * Math.log10(10 - i)) - image.getWidth() / 2, baseY - image.getHeight() / 2, image);
                    ...
                    buttons[3] = new Button((int)(baseX * Math.log10(10000 - 1000*i)) - image.getWidth()/2, baseY - image.getHeight()/2, image);
                    try {
                        Thread.sleep(50);
                ...
                for (int i = 1; i < 9; i++) {
                    ...
                    image = heroes[theOne + 1 < heroes.length ? theOne + 1 : 0].getImage();
                    image = Bitmap.createScaledBitmap(image, (int) (baseWidth * Math.log10(10 - i)), (int) (baseHeight * Math.log10(10 - i)), false);
                    buttons[2] = new Button((int) (baseX * Math.log10(1000 + 1000*i)) - image.getWidth() / 2, baseY - image.getHeight() / 2, image);
                    image = heroes[theOne - 1 < 0 ? (heroes.length > 1 ? heroes.length - 2 : 0): (theOne - 2 < 0 ? heroes.length - 1 : theOne - 2)].getImage();
                    image = Bitmap.createScaledBitmap(image, (int)(baseWidth * Math.log10(1 + i)), (int)(baseHeight * Math.log10(1 + i)), false);
                    buttons[3] = new Button((int)(baseX * Math.log10(1 + i)) - image.getWidth()/2, baseY - image.getHeight()/2, image);
                    try {
                        Thread.sleep(50);
                    } catch (InterruptedException e) {
                        e.printStackTrace();
